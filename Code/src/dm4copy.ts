        ...
        WhoIsX: {
            ...
            LISTEN_COMPLETE: [ 
              {
                actions: assign(({ context }) => { 
                  return { celebrity_reply: context.lastResult}}),
                target: ".Choice"   
                },                                         
          ...
          states:{
            Choice: {
              always: [
                {
                  target: "#DM.Main.WhoIsX.CheckCelebrity",
            ...
            Ask: {
              ...
              on: {
                RECOGNISED: { 
                  actions: assign(({ event }) => { 
                    return { lastResult:event.nluValue.entities[0].text }; 
                  }),                                             
                },
                ASR_NOINPUT: { 
                  actions: assign({ lastResult: null }),
          ...
          states: {
            AskPerson: {
              ...
              on: {
                LISTEN_COMPLETE: [ 
                  /* { 
                    actions: ({ context }) => (context.person_reply = context.lastResult.entities[0].text)  
                    },  */                                            
                  {
                    target: "CheckPerson",
